...
PDF_LIST = []
for r in range(1, 19):
    PDF_LIST.append((1,  f"{r:02d}A", f"{BASE}/-/media/wdts/nsb/pdf/MS-Sample-Questions/Sample-Set-1/m_round{r:02d}.pdf"))
for r in range(1, 18):
    PDF_LIST.append((13, f"{r:02d}A", f"{BASE}/-/media/wdts/nsb/pdf/MS-Sample-Questions/Sample-Set-13/2019-NSB-MSR-Round-{r}A.pdf"))
for r in range(1, 18):
    PDF_LIST.append((14, f"{r:02d}A", f"{BASE}/-/media/wdts/nsb/pdf/MS-Sample-Questions/Sample-Set-14/2020-MS-Rd{r}.pdf"))
for r in range(1, 11):
    PDF_LIST.append((15, f"{r:02d}A", f"{BASE}/-/media/wdts/nsb/pdf/MS-Sample-Questions/Sample-Set-15/Set-{r}-MS-2021.pdf"))
...
def process_pdf(pdf_path, set_num, round_label):
    ...
    while i < len(blocks):
        ...
        if label == "TOSSUP":
            ...
            if tossup:
                ...
                questions.append({
                    "id": q_num,
                    "meta": {"set": set_num, "round": round_label, "questionNum": dn},
                    "tossup": tossup,
...
function getQuestionsBySubject(subject) {
  return questions.filter(q => q.tossup.subject === subject);
}
function checkAnswer(userAnswer, question) {
  const ua = userAnswer.trim().toUpperCase();
  if (question.type === "Multiple Choice") {
    return ua === question.answer || ua === (question.answerText || "");
  }
  const accepted = question.acceptedAnswers
    ? question.acceptedAnswers.map(a => a.toUpperCase())
...
def main():
    ...
    print(f"Processing {len(PDF_LIST)} PDFs...\n")
    for (set_num, round_label, url) in PDF_LIST:
        path = f"pdf_cache/set{set_num}_round{round_label}.pdf"
        if not os.path.exists(path):
            print(f"  Downloading Set {set_num} Round {round_label}...", end=" ", flush=True)
            if not download_pdf(url, path):
                continue
            print("OK")
            time.sleep(0.3)
        else:
            print(f"  Set {set_num} Round {round_label} (cached)", end=" ")
        try:
            parsed = process_pdf(path, set_num, round_label)
            for q in parsed:
                q["id"] = global_id
                global_id += 1
